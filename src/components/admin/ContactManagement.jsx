...
export default function ContactManagement({ contacts, setContacts, fetchPatients }) {
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`https://hospital-portfolio-backend.onrender.com/api/patients/${id}`);
        await fetchPatients();
      } catch (error) {
        console.error(error);
  ...
  const handleResolve = async (id) => {
    try {
      await axios.put(`https://hospital-portfolio-backend.onrender.com/api/patients/${id}/resolve`);
